...
fuzzyData = pd.read_excel('../third_experiment/dataset_24h_apo.xlsx', engine='calamine')
...
for stage in reversed(range(n_stages)):
    ...
    #param_grid = {'max_depth': [5, 6], 'min_samples_split': [2, 3, 4, 5, 6]}
...
def run_test_planner(nsteps=3):
    ...
    ax2.set_xlim(0, 4)
    ax2.plot([0, 1, 2, 3], necrosis[0, 0:4], linestyle='-', marker='o', linewidth=2.1, color='royalblue')
    ax2.plot([0, 1, 2, 3], necrosis[1, 0:4], linestyle='--', marker='o', linewidth=2., color='indianred')
    ax2.plot([0, 1, 2, 3], necrosis[2, 0:4], linestyle='--', marker='o', linewidth=2., color='orange')
    ax2.plot([0, 1, 2, 3], necrosis[3, 0:4], linestyle='--', marker='o', linewidth=2., color='seagreen')
    plt.legend(["DTR", "C1  inhibition", "UPR Activation", "Combined"], loc="lower right")
    plt.show()
    print(necrosis)
    print(apoptosis)
